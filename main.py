...
# 1. API 데이터 조회 함수 (캐싱 적용)
# -------------------------------------------------------------------
# 선택한 날짜(target_dt_str)에 따라 1시간(3600초) 동안 캐시를 유지합니다.
@st.cache_data(ttl=3600)
def fetch_box_office_data(target_dt_str, api_key):
    """KOBIS API를 호출하여 dailyBoxOfficeList 데이터를 반환하는 함수"""
    url = "https://www.kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchDailyBoxOfficeList.json"
...
# -------------------------------------------------------------------
# 2. 날짜 선택기 (한국 시간 KST 기준어제까지 선택 가능)
# -------------------------------------------------------------------
kst = pytz.timezone("Asia/Seoul")
now_kst = datetime.now(kst)
yesterday_kst = (now_kst - timedelta(days=1)).date()  # KST 기준 어제 날짜

st.title("🎬 일별 박스오피스 조회")

# 달력 위젯 생성 (최대 고를 수 있는 날짜는 어제까지)
selected_date = st.date_input(
    "조회할 날짜를 선택해 주세요",
    value=yesterday_kst,
    max_value=yesterday_kst,
    help="오늘 날짜는 아직 집계 전이므로 선택할 수 없습니다.",
)

# API 요청용 포맷(YYYYMMDD)과 화면 표시용 포맷 생성
target_dt = selected_date.strftime("%Y%m%d")
formatted_date = selected_date.strftime("%Y년 %m월 %d일")

st.caption(f"선택한 조회 기준일: {formatted_date}")
...
if "KOBIS_KEY" not in st.secrets:
    st.error("🔑 API 인증키가 설정되지 않았습니다.")
    st.info(
        "Streamlit Cloud의 App Settings > Secrets 메뉴에서 KOBIS_KEY를 등록했는지 확인해 주세요."
    )
    st.stop()
...
if raw_data is None:
    st.error("⚠️ 서버 통신에 실패했습니다.")
    st.warning("인터넷 연결 상태 및 KOBIS 서버 작동 여부를 확인해 주세요.")
    st.stop()

# (2) API 오류 메시지(faultInfo) 응답
if "faultInfo" in raw_data:
    st.error("⚠️ API 호출 오류가 발생했습니다.")
    st.warning(
        f"확인 사항: Secrets의 KOBIS_KEY가 올바른지 확인해 주세요.\n\n"
        f"오류 상세: {raw_data['faultInfo'].get('message', '알 수 없는 오류')}"
    )
    st.stop()

# (3) 영화 목록 데이터 검증 (비어 있는 경우)
box_office_result = raw_data.get("boxOfficeResult", {})
movie_list = box_office_result.get("dailyBoxOfficeList", [])

if not movie_list:
    # 요구사항: 영화 목록이 비어 있으면 "그날은 아직 집계 전입니다" 출력
...
# 숫자형으로 변환할 컬럼 지정 (순위, 순위증감, 관객수, 누적관객, 스크린수)
...
# [요구사항 1] 순위 증감(rankInten) 화살표 포맷 함수
def format_rank_change(val):
    if val > 0:
        return f"🔺 +{val}"  # 순위 상승: 빨간 위 화살표
    elif val < 0:
        return f"🔹 {val}"  # 순위 하락: 파란 아래 기호
    else:
        return "-"  # 변동 없음


df["rankIntenDisplay"] = df["rankInten"].apply(format_rank_change)


# [요구사항 2] 누적 관객수 100만 명 이상 트로피 이모지 추가 함수
...
df["movieNmDisplay"] = df.apply(add_trophy_if_million, axis=1)
...
st.subheader(f"🥇 1위 영화: {top_1_movie['movieNmDisplay']}")
...
st.subheader("📊 관객수 상위 5개 영화")
top5_df = df.head(5)
st.bar_chart(data=top5_df, x="movieNmDisplay", y="audiCnt", color="#FF4B4B")
